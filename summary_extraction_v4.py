...
LANGUAGES: Dict[str, LangConfig] = {
    ...
    "cangjie": LangConfig(
        name="cangjie", extensions=[".cj"],
        module_node="source_file", class_node="class_definition",
        function_node="function_definition", method_node="function_definition",
        comment_prefix="//"
    ),
}
...
def _extract_cangjie_symbols(content: str) -> List[SymbolInfo]:
    """Extract symbols from Cangjie (.cj) files using regex."""
    symbols = []
    lines = content.split("\n")

    # Patterns for Cangjie syntax
    class_pattern = re.compile(r'^(?:public\s+)?(?:open\s+)?class\s+(\w+)(?:\s*<[^>]+>)?(?:\s*\(([^)]*)\))?(?:\s*:\s*([^{]+))?')
    func_pattern = re.compile(r'^(?:public\s+)?(?:static\s+)?func\s+(\w+)(?:\s*<[^>]+>)?\s*\(([^)]*)\)(?:\s*:\s*(\S+))?')
    foreign_func_pattern = re.compile(r'^\s+func\s+(\w+)\s*\(([^)]*)\)(?:\s*:\s*(\S+))?')
    var_pattern = re.compile(r'^(?:public\s+)?(?:let|var)\s+(\w+)\s*(?::\s*(\S+))?')
    ...
    while i < len(lines):
        ...
        if class_match:
            ...
            if class_match.group(3):
                base_classes = [b.strip() for b in class_match.group(3).split(",")]
            ...
            symbols.append(SymbolInfo(
                name=class_name, type="class", code=class_code,
                start_line=start_line, end_line=start_line + len(class_lines) - 1,
                base_classes=base_classes, methods=[], language="cangjie",
            ))
        ...
        # Function definition (top-level or in class)
        ...
        if func_match:
            ...
            symbols.append(SymbolInfo(
                name=func_name, type=symbol_type, code=func_code,
                start_line=start_line, end_line=start_line + len(func_lines) - 1,
                parent_class=current_class, params=f"({params})",
                docstring=docstring, language="cangjie",
            ))
            if current_class:
        ...
        foreign_match = foreign_func_pattern.match(line)
        if foreign_match and "foreign" in "\n".join(lines[max(0,i-5):i]):
            func_name = foreign_match.group(1)
            params = foreign_match.group(2) or ""
            return_type = foreign_match.group(3) or ""

            symbols.append(SymbolInfo(
                name=func_name, type="function", code=line,
                start_line=i+1, end_line=i+1,
                params=f"({params})", language="cangjie",
            ))
            i += 1
            continue
        ...
        if var_match:
            ...
            var_type = var_match.group(2) or ""
            # Only add if it's a top-level constant with a meaningful name
            if var_name.isupper() and len(var_name) > 2:
                symbols.append(SymbolInfo(
                    name=var_name, type="const", code=line,
                    start_line=i+1, end_line=i+1,
                    language="cangjie",
                ))
            i += 1
...
def extract_symbols(file_info: FileInfo) -> List[SymbolInfo]:
    ...
    # Cangjie uses regex extraction (no tree-sitter available)
    if lang_name == "cangjie":
        return _extract_cangjie_symbols(file_info.content)
